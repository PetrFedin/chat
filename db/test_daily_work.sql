...
BEGIN
  ...
  INSERT INTO files(id,organization_id,workspace_id,uploaded_by,name,mime_type,size_bytes,storage_key,status)
    VALUES (file_id,org,ws,member_id,'daily-review.txt','text/plain',42,'daily/review.txt','ready');
  INSERT INTO file_links(organization_id,workspace_id,file_id,entity_type,entity_id,linked_by)
    VALUES (org,ws,file_id,'message',msg,member_id);
  ...
  IF NOT EXISTS (
    ...
    WHERE f.workspace_id=ws AND f.id=file_id AND fl.entity_type='message' AND fl.entity_id=msg
  ) THEN
