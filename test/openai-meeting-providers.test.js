...
test('provider HTTP errors remain structured and do not masquerade as successful processing',async()=>{
  const transcription=new OpenAITranscriptionProvider({
    apiKey:'test-key',
    fetchImpl:async()=>jsonResponse({error:{message:'unsupported media',type:'invalid_request_error',code:'bad_media'}},400),
  });
  await assert.rejects(
    ()=>transcription.transcribe({body:Buffer.from('bad'),storageKey:'bad.mp4'}),
    (error)=>error.code==='OPENAI_TRANSCRIPTION_FAILED'&&error.statusCode===400&&error.details?.providerCode==='bad_media',
  );
  ...
  await assert.rejects(
    ...
    (error)=>error.code==='OPENAI_SUMMARY_FAILED'&&error.statusCode===502,
  );
});
