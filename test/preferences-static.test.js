...
test('theme and locale preference assets are syntactically valid and wired into the shell', async () => {
  const [html, js, contextJs, css, sw] = await Promise.all([
  ...
  assert.doesNotThrow(() => new Function(contextJs));
  assert.match(html, /data-theme="dark"/);
  assert.match(html, /\/preferences\.css/);
  assert.match(html, /<script src="\/preferences\.js"><\/script>[\s\S]*<script src="\/preferences-context\.js"><\/script>[\s\S]*<script src="\/vendor\/livekit-client\.js"><\/script>/);
  assert.match(js, /chat\.theme/);
  assert.match(js, /chat\.locale/);
  assert.match(js, /new Set\(\['dark', 'light'\]\)/);
  assert.match(js, /new Set\(\['ru', 'en'\]\)/);
  assert.match(contextJs, /Company name/);
